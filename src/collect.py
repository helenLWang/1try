...
def consume_events(
    ...
    Returns ratings rows, watch aggregates, user ids, movie ids, and stats.
    Watch events are aggregated in memory so we never write one row per minute.
    """
    ...
    if recent:
        per_part = max(1, max_events // max(len(tps), 1) + 1)
        for tp in tps:
            start = max(begin[tp], end[tp] - per_part)
    ...
    n_watch = n_rate = n_create = n_other = n_bad_rating = 0
        ...
        while n < max_events:
            ...
            for _tp, messages in batch.items():
                for msg in messages:
                    ...
                    n += 1
                    pbar.update(1)
                    event = parse_line(_decode(msg.value))
                    if event is None:
                        n_other += 1
                        continue
    ...
    stats = {
        "events_scanned": n,
        "watch_events": n_watch,
        "rate_events": n_rate,
        "create_account_events": n_create,
        "other_events": n_other,
        "ratings_out_of_range": n_bad_rating,
...
def collect(max_events: int, recent: bool, skip_metadata: bool) -> None:
    _ensure_data_dir()
    print(
        f"Collecting up to {max_events} events from {KAFKA_TOPIC} "
        f"at {KAFKA_BOOTSTRAP} (recent={recent})"
    )
